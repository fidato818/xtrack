...
const Product = ({ id }) => {
  ...
  usage?.map((useg) =>
    useg.name.toUpperCase() == pageProduct.name?.toUpperCase()
      ...
      {pageProduct.length !== 0 ? (
        <>
            ...
            <div className="row">
              <div className="col-lg-6 col-12 d-flex justify-content-center align-items-center ">
                {pageProduct?.assets?.length > 1 ? (
                  ...
                  <img
                    src={pageProduct?.assets[0]?.url}
                    alt={pageProduct.name}
                    className="cursor-p"
                    style={{ height: '350px' }}
                    onClick={() => openInNewTab(pageProduct.assets[0]?.url)}
                  />
                )}
              </div>
              <div className="col-lg-6 col-12 d-flex flex-column pt-4 ps-lg-2 ps-5">
                <div className="row text-small">
                  {pageProduct.inventory.available > 0 ? (
                ...
                <h2 className="text-uppercase">{pageProduct.name}</h2>
                <span className="fs-3 text-danger">
                  {pageProduct.variant_groups[0]?.options[0]?.id == size
                    ? pageProduct.variant_groups[0].options[0].price
                        .formatted_with_code
                    : pageProduct.variant_groups[0]?.options[1]?.id == size
                    ? pageProduct.variant_groups[0].options[1].price
                        .formatted_with_code
                    : pageProduct.price.formatted_with_code}
                </span>
                ...
                <div className="d-flex flex-column">
                  <div className="py-2">
                    <span className="ps-2">
                      {pageProduct.variant_groups[0]?.name}
                    </span>
                    {pageProduct.variant_groups
                      ? pageProduct.variant_groups[0].options.map((option) => {
                          return (
                            <>
                              <div
                                onChange={(e) =>
                                  getVariantSize(
                                    pageProduct.variant_groups[0].id,
                                    e.target.value,
                                  )
                                }
                              >
                  ...
                  <div className="col py-2">
                    <span className="ps-2">
                      {pageProduct.variant_groups[1]?.name}
                    </span>
                    {pageProduct.variant_groups
                      ? pageProduct.variant_groups[1]?.options.map((option) => {
                          return (
                            <>
                              <div
                                onChange={(e) =>
                                  getVariantFlavour(
                                    pageProduct.variant_groups[1].id,
                                    e.target.value,
                                  )
                                }
                              >
                ...
                <div className="p-2">
                  ...
                  >
                    {pageProduct.inventory?.available > 0 ? (
                      pageProduct.variant_groups.length > 1 ? (
                        <button
                          type="button"
                      ...
                      ) : pageProduct.variant_groups.length > 0 ? (
                        <button
                          type="button"
                          className={`btn btn-danger text-uppercase ${disableBtn}`}
                          style={{ width: '80%' }}
                        ...
                        <button
                          ...
                          style={{ width: '80%' }}
                          onClick={() =>
                            context.addToCart(pageProduct.id, quantity)
                          }
                        >
                          Add to cart
                        </button>
          ...
          <div className="container-fluid">
            {pageProduct.related_products.length > 0 ? (
              <>
                <h3 className="text-uppercase py-4 ps-lg-4 ps-2 fw-bold">
                  Related products
                </h3>
                <Slider medium={2} products={pageProduct.related_products} />
              </>
            ) : null}
          </div>
        </>
